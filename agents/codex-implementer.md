...
| `-c model_reasoning_effort=xhigh` | The lane's whole value is maximum-effort GPT-6 Astra. |
...
`--model gpt-6-astra` is the current top GPT tier — if the caller's spec names a different codex model, use that instead; the slug is a documented default, not a constant.
